...
.then(response => {
    ...
    let umbreon = new Pokemon ( data.stats[5].base_stat, data.stats[4].base_stat, data.stats[3].base_stat, array);

    let pokeClass = document.querySelector(".poke");
            pokeClass.innerHTML=`<p class="hp">HP:  ${umbreon.hp}</p><p class="attack">ATTACK: ${umbreon.attack}</p>
                                <p class="defense">DEFENSE: ${umbreon.defense}</p><p class="skills" <strong>ABILITIES: <br> </strong>${umbreon.ability}</p>`;
